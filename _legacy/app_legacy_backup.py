...
with tab_edit:
    ...
    if not _pdfs:
        ...
        from pagewrite import page_canvas, save_overlay

        with st.expander("✍️ Write on this page — pen / stylus / touch",
                         expanded=True):
            st.caption("The page itself is the canvas: draw or write "
                       "directly on it. Hit **Apply to page** once — "
                       "only that tap reruns the app.")
            _pw, _ph = ed.page_size(e_page)
            _disp_h = 900 if _ph >= _pw else 640
            try:
                _bg = ed.render_raw_preview(e_page, dpi=110)
            except Exception:
                _bg = ed.render_preview(e_page, dpi=110)
            try:
                _val = page_canvas(_bg, _disp_h, _pw, _ph,
                                   key=f"pw_{e_src}_{e_page}")
            except Exception as e:
                st.warning(f"Page canvas unavailable ({e}).")
                _val = None
            if _val:
                try:
                    _ovl, _sx, _sy = save_overlay(_val)
                except Exception:
                    _ovl, _sx, _sy = None, None, None
                if _ovl:
                    ed.add_page_ink(e_page, _ovl, _sx or 1.0, _sy or 1.0)
                    st.success("Ink applied to the page — "
        ...
        _WB_MODE = st.radio("Board mode", ["One-tap (new)", "Classic"],
                            horizontal=True, key="ed_wb_mode")

        if _WB_MODE.startswith("One-tap"):
            from whiteboard_tap import one_tap_board_panel

            with st.expander("One-tap board — draw & stamp", expanded=True):
                st.caption("Write with your **pen / stylus / touch / mouse**, "
                           "then hit **Stamp into PDF** on the canvas — "
                           "no download step. Drawing never reruns the app.")
                try:
                    wb_file = one_tap_board_panel()
                except Exception as e:
                    st.warning(f"One-tap board unavailable ({e}); "
                               "use Classic below.")
                    wb_file = None
                b1, b2 = st.columns([1, 1])
                if b1.button("Stamp board into PDF",
                             key="ed_wb_stamp", disabled=not wb_file):
                    if wb_file:
                        dst = os.path.join(UPLOAD_DIR,
                                           os.path.basename(wb_file))
                        if dst != wb_file:
                            open(dst, "wb").write(open(wb_file, "rb").read())
                        ed.add_board(e_page, dst)
                        st.session_state.pop("wb_tap_file", None)
                        st.rerun()
                if b2.button("Discard board", key="ed_wb_clear",
        ...
        with st.expander("Classic board — download & upload",
                         expanded="Classic" in _WB_MODE):
            st.caption("Fallback: draw, **Download PNG**, re-upload it, "
                       "then stamp. Works everywhere.")
            ...
            if wb1.button("📌 Stamp board as full page",
                          key="ed_wb_stamp_classic", disabled=not wb_file):
                if wb_file:
                    ...
                    ed.add_board(e_page, dst)
                    st.rerun()
            if wb2.button("🧽 Discard board", key="ed_wb_clear_classic",
                          disabled=not wb_file):
                try:
                    os.remove(wb_file)
                except OSError:
